...
from NeRF.nerf.render.render import OctreeRender_trilinear_fast as renderer
from NeRF.nerf.render.render import evaluation
from NeRF.nerf.render.util.Reg import TVLoss, compute_dist_loss
from NeRF.nerf.render.util.Sampling import GM_Resi, cal_n_samples
from NeRF.nerf.render.util.util import N_to_reso
from NeRF.nerf.render.util.learn_pose import LearnPose
from NeRF.nerf.dataloader.ray_utils import get_rays
...
class Trainer:
    ...
    def sample_data(self, train_dataset, test_dataset, iteration, pose_net, optim_counter):
        """
        Sample a batch of data from the dataset.
        """
        train_depth = None
        # sample rays: shuffle all the rays of training dataset and sampled a batch of rays from them.
        #TODO: adapt code for rays and hierarchical sampling
        if self.cfg.data.datasampler_type == "rays":
            ...
            proportion_static = 1.0 #todo: change
        # sample images: randomly pick one image from the training dataset and sample a batch of rays from all the rays of the image.
            ...
            if (optim_counter < self.cfg.optim.change_train_mode or
                self.cfg.optim.change_train_mode == -1):
                img_i = self.sampler.nextids()
                data = train_dataset[img_i]
            else:
                img_i = self.sampler_poses.nextids()
                if img_i < len(train_dataset):
                    data = train_dataset[img_i]
                else:
                    data = test_dataset[img_i - len(train_dataset)]
            rgb_train, frame_time, dynamic_mask = (
                data["rgbs"].to(self.device).view(-1, 3),
                data["time"].squeeze(0),
                data["masks"].squeeze(0)
            )

            proportion_static = torch.sum(dynamic_mask == 0) / len(dynamic_mask)

            # Create new rays based on refined poses
            if (optim_counter >= self.cfg.optim.change_train_mode and
                self.cfg.optim.change_train_mode != -1):
                pose = pose_net(img_i.item())
            else:
                with torch.no_grad():
                    pose = pose_net(img_i.item())

            rays_o, rays_d = get_rays(train_dataset.directions, pose)
            rays_concat = torch.cat([rays_o, rays_d], 1)

            # Detach dynamic pixels
            rays_train = rays_concat.clone()
            rays_train[dynamic_mask] = rays_train[dynamic_mask].detach()
        ...
        # hierarchical sampling from dyNeRF: hierachical sampling involves three stages of samplings.
            ...
            if iteration <= self.cfg.data.stage_1_iteration:
                ...
                rgb_train = rgb_train[select_inds]
                frame_time = torch.ones_like(rays_train[:, 0:1]) * frame_time
        return rays_train, rgb_train, frame_time, train_depth, proportion_static

    def init_sampler(self, train_dataset, test_dataset):
        """
        ...
        if self.cfg.data.datasampler_type == "rays":
            ...
            self.sampler_poses = SimpleSampler(len(train_dataset) + len(test_dataset), 1)
        elif self.cfg.data.datasampler_type == "hierach":
    ...
    def train(self):
        ...
        all_poses = torch.cat((train_dataset.poses, test_dataset.poses))
        pose_net = LearnPose(len(train_dataset.poses) + len(test_dataset.poses),
                             self.cfg.optim.learn_R,
                             self.cfg.optim.learn_t,
                             all_poses)
        ...
        self.init_sampler(train_dataset, test_dataset)
        ...
        optimizer_pose = torch.optim.Adam(
            pose_net.parameters(), lr=self.cfg.optim.pose_lr
        )
        scheduler_pose = torch.optim.lr_scheduler.MultiStepLR(
            optimizer_pose,
            milestones=self.cfg.optim.pose_milestones,
            gamma=self.cfg.optim.pose_lr_gamma
        )
        optim_counter = 0
        iteration_decay = 0
        ...
        for iteration in pbar:
            ...
            # Sample data
            rays_train, rgb_train, frame_time, depth, proportion_static = self.sample_data(
                train_dataset, test_dataset, iteration, pose_net, optim_counter
            )
            ...
            # Scale learning rate by proportion of static pixels
            for param_group in optimizer_pose.param_groups:
                param_group['lr'] /= proportion_static
            ...
            lr_factor = self.get_lr_decay_factor(iteration_decay)
            ...
            # Reset learning rate to follow scheduler
            for param_group in optimizer_pose.param_groups:
                param_group['lr'] = scheduler_pose.get_last_lr()[0]
            ...
            optim_counter += 1
            if (optim_counter == 2 * self.cfg.optim.change_train_mode and
                self.cfg.optim.change_train_mode != -1):
                optim_counter = 0
                for param_group in optimizer_pose.param_groups:
                    param_group['lr'] = self.cfg.optim.pose_lr
                scheduler_pose = torch.optim.lr_scheduler.MultiStepLR(
                    optimizer_pose,
                    milestones=self.cfg.optim.pose_milestones,
                    gamma=self.cfg.optim.pose_lr_gamma
            ...
            if (
                ...
                # Calculate new rays based on refined poses
                all_rays = []
                idx_start = len(train_dataset.poses)
                pose_net.eval()
                for i in range(len(test_dataset.poses)):
                    with torch.no_grad():
                        rays_o, rays_d = get_rays(test_dataset.directions,
                                                  pose_net(idx_start + i))
                        all_rays += [torch.cat([rays_o, rays_d], 1)]
                test_dataset.all_rays = torch.stack(all_rays, 0)
