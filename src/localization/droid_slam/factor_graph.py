...
class FactorGraph:
    ...
    def rm_keyframe(self, ix):
        ...
        with self.video.get_lock():
            ...
            self.video.segmentation_masks[ix] = self.video.segmentation_masks[ix+1]
    ...
    def update(self, t0=None, t1=None, itrs=2, use_inactive=False, EP=1e-7, motion_only=False):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            #! Dynamic Masking of Optical Flow
            dynamic_masks = F.interpolate(self.video.segmentation_masks.float(), size=self.target.shape[2:4], mode='bilinear').bool()
            combined_masks = (dynamic_masks[self.ii] | dynamic_masks[self.jj]).permute(1, 0, 2, 3)
            self.weight[combined_masks] = 0

            dynamic_masks = F.interpolate(self.video.motion_masks.float(), size=self.target.shape[2:4], mode='bilinear').bool()
            combined_masks = (dynamic_masks[self.ii] | dynamic_masks[self.jj]).permute(1, 0, 2, 3)
            self.weight[combined_masks] = 0
    ...
    @torch.cuda.amp.autocast(enabled=True)
    def update_non_kfs(self, t0=None, t1=None, itrs=2, use_inactive=False, EP=1e-7, motion_only=False, kf=None, non_kf=None):
        """ run update operator on factor graph """
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            #! Dynamic Masking of Optical Flow
            dynamic_masks_kfs = F.interpolate(self.video.segmentation_masks.float(), size=self.target.shape[2:4], mode='bilinear').bool()
            dynamic_masks_non_kfs = F.interpolate(self.video.all_segmentation_masks.float(), size=self.target.shape[2:4], mode='bilinear').bool()
            if self.ii.shape[0] == 32:
                combined_masks = (dynamic_masks_kfs[self.ii] | dynamic_masks_non_kfs[torch.cat((non_kf, non_kf), 0)]).permute(1, 0, 2, 3)
            else:
                additional = self.ii.shape[0] - non_kf.shape[0]
                combined_masks = (dynamic_masks_kfs[self.ii] | dynamic_masks_non_kfs[torch.cat((non_kf, non_kf[0:additional]), 0)]).permute(1, 0, 2, 3)
            self.weight[combined_masks] = 0

            dynamic_masks_kfs = F.interpolate(self.video.motion_masks.float(), size=self.target.shape[2:4], mode='bilinear').bool()
            dynamic_masks_non_kfs = F.interpolate(self.video.all_motion_masks.float(), size=self.target.shape[2:4], mode='bilinear').bool()
            if self.ii.shape[0] == 32:
                combined_masks = (dynamic_masks_kfs[self.ii] | dynamic_masks_non_kfs[torch.cat((non_kf, non_kf), 0)]).permute(1, 0, 2, 3)
            else:
                combined_masks = (dynamic_masks_kfs[self.ii] | dynamic_masks_non_kfs[torch.cat((non_kf, non_kf[0:additional]), 0)]).permute(1, 0, 2, 3)
    ...
    def update_lowmem(self, t0=None, t1=None, itrs=2, use_inactive=False, EP=1e-7, steps=8):
        ...
        for step in range(steps):
            ...
            for i in range(0, self.jj.max()+1, s):
                ...
                #! Dynamic Masking of Optical Flow
                dynamic_masks = F.interpolate(self.video.segmentation_masks.float(), size=self.target.shape[2:4], mode='bilinear').bool()
                combined_masks = (dynamic_masks[iis] | dynamic_masks[jjs]).permute(1, 0, 2, 3)
                weight[combined_masks] = 0
