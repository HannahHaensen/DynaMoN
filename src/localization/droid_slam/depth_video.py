...
class DepthVideo:
    def __init__(self, image_size=[480, 640], buffer=1024, stereo=False, device="cuda:0"):
        ...
        self.segmentation_masks = torch.zeros(buffer, 1, ht, wd, device="cuda", dtype=torch.bool).share_memory_()
        self.motion_masks = torch.zeros(buffer, 1, ht, wd, device="cuda", dtype=torch.bool).share_memory_()
        # max 5000 images as placeholders for sizes of masks
        self.all_segmentation_masks = torch.zeros(5000, 1, ht, wd, device="cuda", dtype=torch.bool).share_memory_()
    ...
    def __item_setter(self, index, item):
        ...
        if len(item) > 9:
            self.segmentation_masks[index] = item[9]
        
        if len(item) > 10:
            self.motion_masks[index] = item[10]
    ...
    def __getitem__(self, index):
        ...
        with self.get_lock():
            ...
            item = (
                ...
                self.nets[index],
                self.inps[index],
                self.segmentation_masks[index],
                self.motion_masks[index])
